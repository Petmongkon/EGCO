...
<div class="ui grid"> 
    ...
    <div class="ui buttons"> 
        <button class="large ui teal button" v-on:click="editContact">Submit</button> 
...
export default { 
  ...
  methods: { 
    editContact () { 
      ...
      console.log(newUser) 
      axios.post('http://localhost:3000/users/'+this.uid, newUser) 
        .then((response) => { 
  ...
  mounted (){
    if(localStorage.getItem('token')==null)
    {
      ...
      axios.get('http://localhost:3000/users/' + this.uid)
        .then((response)=>{
